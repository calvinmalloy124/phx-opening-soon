...
def main():
    ...
    except Exception:  # corrupted tracking file -> reparse everything
        sc_seen = set()
    ...
    # ---- Mesa (Legistar API) ----
    try:
        for rec in mesa.fetch_all():
            rec["key"] = "mesa-" + hashlib.sha1(rec["app_id"].encode()).hexdigest()[:10]
            seen.append(rec)
    except Exception as e:  # noqa
        errors.append(f"mesa: {e}")
    # keep previously-known agenda-based records (Scottsdale/Mesa) that are still inside their active window
    for k, v in known.items():
        if v.get("city") in ("Scottsdale", "Mesa") and k not in {r["key"] for r in seen} and scottsdale.is_active(v):
            seen.append(v)
    ...
    for k, v in known.items():
        v["active"] = k in live_keys and (v.get("city") == "Phoenix" or scottsdale.is_active(v))
